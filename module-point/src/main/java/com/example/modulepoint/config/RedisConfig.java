...
public class RedisConfig {
	...
	@Bean
	public RedisTemplate<String, RefundDto> refundRedisTemplate(RedisConnectionFactory blockingRedisConnectionFactory) {
		RedisTemplate<String, RefundDto> template = new RedisTemplate<>();
		template.setConnectionFactory(blockingRedisConnectionFactory);
		template.setKeySerializer(new StringRedisSerializer());
		template.setValueSerializer(new Jackson2JsonRedisSerializer<>(RefundDto.class));
